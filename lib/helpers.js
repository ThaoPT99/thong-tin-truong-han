 ...
 * 
 * Fix H1: insert trước → verify → delete cũ (tránh mất data nếu insert fail)
 */
async function replaceChildTable(table, schoolId, items) {
  if (!items || !Array.isArray(items)) return;

  // Đọc danh sách ID cũ trước khi thay đổi
  ...
  if (insErr) {
    console.error(`insert ${table} error:`, insErr.message);
    return; // Insert fail → dữ liệu cũ vẫn còn nguyên, an toàn
  }

  // Insert thành công → xoá rows cũ
  if (oldIds.length > 0) {
    const { error: delErr } = await supabase.from(table).delete().in('id', oldIds);
    if (delErr) console.error(`delete old ${table} error:`, delErr.message);
  }
}
