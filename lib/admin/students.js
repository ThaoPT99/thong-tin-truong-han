...
module.exports = requireAdmin(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      // (Applications view removed — feature xoá)

      // View Checklist Progress (tiến độ hồ sơ của học sinh)
      if (view === 'checklist-progress') {
        if (!isDirector) return res.status(403).json({ error: 'Only director can view checklist progress' });

        const { data: progress, error } = await supabase
          .from('student_checklist_progress')
          .select('*, student_profiles!inner(id, email, full_name, phone, last_active)')
      ...
      // View Student Documents (bản nháp Study Plan, giấy tờ)
      if (view === 'student-documents') {
        if (!isDirector) return res.status(403).json({ error: 'Only director can view student documents' });

        const { student_id } = req.query;
        let query = supabase
          .from('student_documents')
          .select('*, student_profiles!inner(id, email, full_name, phone)')
          .order('updated_at', { ascending: false });
      ...
      if (view === 'chat-history') {
        if (!isDirector) return res.status(403).json({ error: 'Only director can view chat history' });

        const { student_id, student_email, limit } = req.query;

        // If email is provided, first look up student_profiles
        let profileIds = [];
        if (student_email) {
        ...
        let query = supabase
          ...
          .select('*, student_profiles!inner(id, email, full_name)')
          .order('created_at', { ascending: false })
          .limit(parseInt(limit) || 50);

        if (student_id) query = query.eq('student_id', student_id);
        if (profileIds.length > 0) query = query.in('student_id', profileIds);
      ...
      // View Activity Logs (hành vi người dùng trên site)
      if (view === 'activity-logs') {
        if (!isDirector) return res.status(403).json({ error: 'Only director can view activity logs' });

        const { student_id, student_email, activity_type, limit } = req.query;
    ...
    if (req.method === 'PUT') {
      ...
      // (Application update view removed — feature xoá)
