...
const { logServerError } = require('../error-logger');
const { insertChildTable, replaceChildTable, replacePartners, upsertAdvisorProfile } = require('../helpers');
...
module.exports = requireAdmin(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const { data, error } = await supabase
        .from('schools')
        .select('id, slug, name, name_kr, name_en, system, quota, region, location, is_active, updated_at')
        .order('slug');
    ...
    if (req.method === 'POST') {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          application_form_url: body.applicationFormUrl || '',
    ...
    if (req.method === 'PUT' || req.method === 'DELETE') {
      ...
      const updateData = {
        ...
        application_form_url: val('applicationFormUrl', existingSchool.application_form_url || ''),
        ...
        is_active: val('isActive', existingSchool.is_active !== false),
