...
/**
 * Gửi thông báo khi có tài khoản mới đăng ký trên web
 */
async function sendNewAccountAlert(accountInfo) {
  const chatId = getAdminChatId();
  if (!chatId) return null;

  const { fullName, phone, email, createdAt } = accountInfo;

  const text = `🆕 <b>Tài khoản mới đăng ký!</b>

👤 Họ tên: ${fullName || 'Không rõ'}
📞 SĐT: ${phone || 'Không rõ'}
📧 Email: ${email || 'Không rõ'}
🕐 Thời gian: ${createdAt || new Date().toLocaleString('vi-VN')}

<i>Đã đăng ký thành công trên web.</i>`;

  return await sendTelegramMessage(chatId, text);
}

/**
 * Gửi thông báo khi học sinh tự gửi form Tư vấn (advisor submission)
 */
async function sendNewAdvisorSubmissionAlert(submissionInfo) {
  const chatId = getAdminChatId();
  if (!chatId) return null;

  const { email, fullName, gender, age, gpa, korean, region, topSchools } = submissionInfo;
  
  const genderText = gender === 'female' ? 'Nu' : gender === 'male' ? 'Nam' : 'Khong ro';
  const regionText = region && region !== 'any' ? region : 'Khong uu tien';
  const topSchoolText = (topSchools || []).slice(0, 3).map(function(s) {
    return s.name + ' (' + (s.score || '?') + '%)';
  }).join(', ') || 'Dang phan tich...';

  const text = '📋 <b>Hoc sinh moi gui form Tu van!</b>\n\n' +
    '👤 Email: ' + (email || 'Khong ro') + '\n' +
    (fullName ? '📝 Ten: ' + fullName + '\n' : '') +
    '👤 Gioi tinh: ' + genderText + ' | Tuoi: ' + (age || '?') + '\n' +
    '📊 GPA: ' + (gpa || '?') + ' | Tieng Han: ' + (korean || 'Chua co') + '\n' +
    '📍 Khu vuc: ' + regionText + '\n' +
    '🏆 Top truong: ' + topSchoolText + '\n\n' +
    '<i>Xem chi tiet: thongtintruonghan.vercel.app/admin/students.html</i>';
