...
- Phù hợp với đối tượng học sinh muốn đi du học Hàn Quốc (visa D-4-1, D-2, D2-6)
- Không dùng emoji, không xuống dòng quá nhiều

Trả về JSON:
{"intro":"Đoạn giới thiệu trường...","suggestedAdvantages": ["Ưu điểm 1","Ưu điểm 2","Ưu điểm 3"]
}`;
...
async function handleTelegramStart(chatId) {
 const text = ` <b>Chào mừng bạn đến với Bot hỗ trợ!</b>Bot này giúp bạn quản lý thông tin du học Hàn Quốc (visa D-4-1, D-2, D2-6).

<b>Các lệnh có sẵn:</b><code>/truong [tên]</code>— Tra cứu thông tin trường
 <code>/danhsach</code>— Danh sách tất cả trường
 <code>/sosanh [t1], [t2]</code>— So sánh 2 trường
 <code>/baocao</code>— Báo cáo tổng quan hôm nay
 <code>/dieukien</code>— Checklist visa du học (D-4-1, D-2)
...
async function handleTelegramHelp(chatId) {
 const text = ` <b>Hướng dẫn sử dụng Bot</b><b>Tra cứu trường</b><code>/truong Osan</code>— Chi tiết 1 trường (tên, học phí, KTX, điều kiện...)
<code>/danhsach</code>— Xem tất cả 18 trường (phân theo khu vực)
<code>/sosanh Osan, Induk</code>— So sánh 2 trường

 <b>Thống kê</b><code>/baocao</code>— Báo cáo hôm nay (lượt xem, IP mới, top trường)

 <b>Visa & Hồ sơ</b><code>/dieukien</code>— Checklist visa du học (D-4-1, D-2), phỏng vấn

 <b>Quản lý học sinh</b><code>/gui Nguyễn Văn A, 0978123456, Osan</code>— Thêm học sinh vào CRM

🧠 <b>Phân tích AI</b><code>/phan-tich</code>— Phân tích IP bằng AI, phát hiện đối tác tiềm năng

ℹ️ <b>Khác</b><code>/thongtin</code>— Thông tin hệ thống, liên hệ
<code>/start</code>— Xem lại menu

 <b>Cảnh báo tự động:</b>• Khi có IP mới từ thành phố lạ
...
<b>Điều kiện:</b>• ${conditions}

<b>Chuyên ngành:</b>${majors}

<b>Ưu điểm:</b>• ${advantages}
...
async function handleTelegramReport(chatId) {
 ...
 const text = ` <b>Báo cáo ngày ${today}</b>Lượt xem: <b>${totalViews}</b>Tìm kiếm: <b>${totalSearches}</b>Phiên: <b>${totalSessions}</b>IP mới: <b>${newIps}</b>${schoolLines ? ` <b>Top trường:</b>${schoolLines}` : ''}

${cityLines ? ` <b>Địa điểm mới:</b>${cityLines}` : ''}
...
`;
 ...
 text += ` Tra cứu chi tiết: <code>/truong [tên]</code>So sánh: <code>/sosanh [t1], [t2]</code>`;
...
async function handleTelegramCompare(chatId, args) {
 ...
 const text = ` <b>So sánh:</b>${escapeHtmlTelegram(a.name)} vs ${escapeHtmlTelegram(b.name)}

<b>Vị trí</b>• ${escapeHtmlTelegram(a.name)}: ${a.region}
• ${escapeHtmlTelegram(b.name)}: ${b.region}

<b>Hệ đào tạo</b>• ${escapeHtmlTelegram(a.name)}: ${a.system}
• ${escapeHtmlTelegram(b.name)}: ${b.system}

<b>Học phí</b>• ${escapeHtmlTelegram(a.name)}: ${a.tuition}
• ${escapeHtmlTelegram(b.name)}: ${b.tuition}

<b>KTX</b>• ${escapeHtmlTelegram(a.name)}: ${a.ktx}
• ${escapeHtmlTelegram(b.name)}: ${b.ktx}

<b>Chỉ tiêu</b>• ${escapeHtmlTelegram(a.name)}: ${a.quota}
• ${escapeHtmlTelegram(b.name)}: ${b.quota}

<b>Điều kiện</b>• ${escapeHtmlTelegram(a.name)}: ${a.conditions}
...
async function handleTelegramVisaChecklist(chatId) {
 ...
 } let text = ` <b>Checklist visa du học (D-4-1, D-2, D2-6)</b>`;
...
async function handleTelegramSystemInfo(chatId) {
 ...
 const text = `ℹ️ <b>Thông tin hệ thống</b>Số trường: <b>${schoolCount}</b>Kỳ hiện tại: <b>${activeSem ? escapeHtmlTelegram(activeSem.title || `Kỳ ${activeSem.ky}/${activeSem.nam}`) : 'Chưa cập nhật'}</b>Lượt xem hôm nay: <b>${todayViews}</b><b>Liên hệ</b>Xử lý visa du học (D-4-1, D-2, D2-6)
 Zalo: Tham gia nhóm trên web

🌐 <a href="https://thongtintruonghan.vercel.app/">thongtintruonghan.vercel.app</a>Gõ <code>/help</code>để xem hướng dẫn chi tiết.`;
...
async function handleTelegramIpAnalysis(chatId) {
 ...
 // Build prompt
 const ipSummary = ipAnalysis.map((ip, i) =>`IP ${i + 1}: ${ip.ip}
 ...
 const systemPrompt = `Bạn là chuyên gia phân tích dữ liệu khách hàng cho một doanh nghiệp xử lý visa du học Hàn Quốc (visa D-4-1, D-2, D2-6).
...
async function handleChatWeb(req, res) {
 ...
 const systemPrompt = `Bạn là trợ lý AI của website Thông Tin Trường Hàn (thongtintruonghan.vercel.app) — chuyên về visa du học Hàn Quốc (D-4-1, D-2, D2-6).
...
7. Nếu không có thông tin → nói"Thông tin này chưa có trong dữ liệu hiện tại"8. Kết thúc gợi ý: mời vào web xem chi tiết hoặc tham gia group Zalo
...
QUY TẮC (theo yêu cầu của Đại sứ quán Hàn Quốc):
- VIẾT TAY (handwritten) — Study Plan bắt buộc phải viết tay, không đánh máy
- Ngôn ngữ: LUÔN ưu tiên tiếng Hàn (vì đây là visa du học Hàn Quốc), chỉ dùng tiếng Anh nếu học sinh theo chương trình tiếng Anh
- Chi tiết, cụ thể, có mốc thời gian rõ ràng (theo từng giai đoạn 6 tháng, 1 năm...)
- Cá nhân hoá theo thông tin học sinh (KHÔNG sao chép mẫu)
- Độ dài: 500-800 từ
- Kết thúc bằng CHỮ KÝ TAY và cam kết tuân thủ luật pháp, về nước đúng hạn
- TUYỆT ĐỐI KHÔNG đề cập vấn đề tài chính trong Study Plan (tài chính là phần riêng của hồ sơ)
- TUYỆT ĐỐI KHÔNG thể hiện ý định định cư hoặc ở lại Hàn Quốc làm việc sau tốt nghiệp
- Thể hiện rõ kế hoạch về nước sau khi hoàn thành chương trình học
- Viết bằng văn phong trang trọng, lịch sự, thể hiện sự chân thành`,
...
- GPA: ${p.gpa || 'Không rõ'}
- Trình độ tiếng Hàn: ${p.koreanLevel || 'Chưa có'}${p.hasTopik && p.topikGrade ? ` (chứng chỉ: ${p.topikGrade})` : ''}${p.ieltsScore ? ` | IELTS: ${p.ieltsScore}` : ''}
...
- MỐC THỜI GIAN: gap >6 tháng nên giải trình, gap >1 năm BẮT BUỘC giải trình + cần chứng từ kèm theo (HĐLĐ, bảng điểm, bệnh án...)
...
- Nếu có chứng từ (HĐLĐ, chứng chỉ, bảng điểm), nêu rõ để tăng độ thuyết phục
- Độ dài: 200-400 từ`,
...
${p.workCompany ? `- Công ty đã làm: ${p.workCompany}` : ''}
${p.workPosition ? `- Vị trí: ${p.workPosition}` : ''}
${p.workDuration ? `- Thời gian: ${p.workDuration} năm` : ''}
...
Viết giải trình cho học sinh này.`
 ...
 // Study Plan can bang cao hon de AI viet sang tao hon; gap/rejection can do chinh xac hon
 var temperature = (type === 'study_plan') ? 0.6 : 0.4;
 ...
 { temperature: temperature, maxTokens: 1500, timeout: 30000 }
 );
 ...
 var systemPrompt = `Bạn là nhân viên phỏng vấn visa Hàn Quốc tại KVAC. Bạn cần phỏng vấn học sinh xin visa ${profile.visaType || 'D-4-1'}.

THÔNG TIN HỌC SINH:
- Họ tên: ${profile.fullName}
- Visa: ${profile.visaType || 'D-4-1'}
- Học vấn: ${profile.educationLevel === 'university'? 'Đại học': 'THPT'}${profile.gpa ? '\n- GPA: '+ profile.gpa : ''}
- Tiếng Hàn: ${profile.koreanLevel || 'Chưa có'}${profile.gapYears >0 ? '\n- Gap year: '+ profile.gapYears + 'năm': ''}${profile.hasVisaRejection ? '\n- Đã từng trượt visa: Có': ''}
${profile.chosenSchool ? `- Trường dự định: ${profile.chosenSchool}` : ''}
${profile.chosenMajor ? `- Ngành dự định: ${profile.chosenMajor}` : ''}

NHIỆM VỤ:
Bạn sẽ phỏng vấn học sinh bằng tiếng Việt. Mỗi lần chỉ hỏi 1 câu. Tổng cộng 6-8 câu hỏi.

CÁC CHỦ ĐỀ CẦN HỎI (theo thứ tự):
1. Giới thiệu bản thân & mục đích du học
2. Lý do chọn Hàn Quốc (không phải nước khác)
3. Lý do chọn trường & ngành học
4. Kế hoạch học tập cụ thể (theo giai đoạn: 6 tháng, 1 năm...)
5. Kế hoạch sau khi tốt nghiệp (về nước làm gì)
6. Tài chính & người bảo lãnh (nguồn tiền, thu nhập người bảo lãnh)
7. (Nếu có gap year) Giải thích khoảng trống thời gian & bằng chứng
8. (Nếu đã trượt visa) Nguyên nhân trượt và cách khắc phục
9. Dự định làm thêm (nếu có) — công việc gì, bao nhiêu giờ, có xin phép không

QUY TẮC:
- Hỏi bằng tiếng Việt, thân thiện nhưng chuyên nghiệp
- KHÔNG hỏi quá nhiều cùng lúc (chỉ 1 câu)
- Điều chỉnh độ khó câu hỏi dựa trên hồ sơ học sinh
- Khi học sinh trả lời xong, bạn đánh giá ngắn gọn và hỏi câu tiếp theo
- Nếu hồ sơ có yếu tố rủi ro (gap >1 năm, trượt visa, tài chính yếu) → hỏi sâu hơn

Trả về KẾT QUẢ DƯỚI DẠNG JSON, KHÔNG có text khác:
{"question":"Câu hỏi phỏng vấn...","questionNumber": 1,"totalQuestions": 7,"category":"gioi-thieu","hint":"Gợi ý trả lời..."}`;

 var result = await callDeepSeek(
 [{ role: 'system', content: systemPrompt }, { role: 'user', content: 'Hãy bắt đầu phỏng vấn học sinh. Hãy hỏi câu hỏi ĐẦU TIÊN.'}],
 ...
 question: parsed.question || 'Hãy giới thiệu về bản thân và mục đích du học Hàn Quốc?',
 ...
 var systemPrompt = `Bạn là nhân viên phỏng vấn visa Hàn Quốc tại KVAC. Đánh giá câu trả lời của học sinh.
...
CÂU TRẢ LỜI MỚI:"${answer}"NHIỆM VỤ:
1. Đánh giá câu trả lời của học sinh (2-3 câu, bằng tiếng Việt, mang tính xây dựng)
2. Cho điểm từ 1-10 (dựa trên: sự trung thực, nhất quán với hồ sơ, tính thuyết phục)
3. Hỏi câu tiếp theo (chỉ 1 câu) — dựa trên chủ đề phù hợp tiếp theo

Tiêu chí đánh giá:
- Câu trả lời có trung thực và nhất quán với hồ sơ không?
- Có thể hiện được mục đích du học rõ ràng không?
- Có trả lời trực tiếp vào câu hỏi không (không lan man)?

Trả về KẾT QUẢ DƯỚI DẠNG JSON:
{"feedback":"Nhận xét về câu trả lời... (2-3 câu, tinh thần xây dựng)","score": <1-10>,"nextQuestion":"Câu hỏi tiếp theo...","questionNumber": ${qNum + 1},"totalQuestions": ${totalQ},"category":"...","hint":"Gợi ý trả lời..."}`;

 var result = await callDeepSeek(
 [{ role: 'system', content: systemPrompt }, { role: 'user', content: 'Hãy đánh giá câu trả lời và hỏi câu tiếp theo.'}],
 ...
 var systemPrompt = `Bạn là nhân viên phỏng vấn visa Hàn Quốc tại KVAC. Tổng kết buổi phỏng vấn.
...
NHIỆM VỤ:
Tổng kết buổi phỏng vấn, đánh giá tổng thể và đưa ra lời khuyên.

Tiêu chí đánh giá tổng thể:
- Học sinh có trả lời nhất quán và trung thực không?
- Mục đích du học có rõ ràng không?
- Kế hoạch tương lai có thuyết phục không (đặc biệt là kế hoạch về nước)?
- Tài chính có được giải trình tốt không?
- Thái độ có tự tin, nghiêm túc không?

Trả về JSON:
{"overallScore": <1-10>,"overallFeedback":"Nhận xét tổng thể... (2-3 câu)","strengths": ["Điểm mạnh 1","Điểm mạnh 2","Điểm mạnh 3"],"weaknesses": ["Điểm yếu 1","Điểm yếu 2"],"tips": ["Lời khuyên 1","Lời khuyên 2","Lời khuyên 3"]
}`;

 var result = await callDeepSeek(
 [{ role: 'system', content: systemPrompt }, { role: 'user', content: 'Hãy tổng kết buổi phỏng vấn và cho lời khuyên.'}],
 { temperature: 0.4, maxTokens: 1000, timeout: 20000 }
 );
