...
function exportComparePDF(container) {
 ...
 }</style><head><body><h1>So sánh trường Hàn Quốc</h1><p>Ngày xuất: ${new Date().toLocaleString("vi-VN")}</p>${resultEl.innerHTML}</body><html>`;
...
function updatePageMeta(viewId, school) {
 ...
 const semester = typeof SEMESTER_INFO !=="undefined"? `Kỳ tháng ${SEMESTER_INFO.ky ||"3"}/${SEMESTER_INFO.nam ||"2027"}`
 : `Visa ${visaType}`;
 const title = school
...
function renderCompareResult(container) {
 ...
 ${renderCompareRow("Rủi ro cần lưu ý", schools, s =>renderText(getCompareRisk(s)))}</tbody><table><div>`;
...
function renderAdvisorMetrics(school) {
 ...
 return `
 Visa ${r.visaChance || 3}/5<br>Việc làm ${r.jobOpportunity || 3}/5<br>E7 ${r.e7Opportunity || 3}/5<br>Chi phí ${r.costLevel || 3}/5
 `;
}
...
function renderD26Checklist() {
 ...
 return `
 <section class="d26-checklist"><div class="checklist-head"><div><p class="advisor-kicker">Bộ hồ sơ mẫu sạch</p><h2>Checklist hồ sơ Visa D2-6</h2><p>Tổng hợp các giấy tờ cần chuẩn bị khi làm hồ sơ D2-6, chia theo từng nhóm để dễ kiểm tra và theo dõi tiến độ.</p><div><span>${total} đầu mục</span><div><div class="checklist-groups">${groups.map(function(group, groupIndex) {
 return `
...
function renderExtra() {
 ...
 return `
 <section class="docs-view">${renderD26Checklist()}
 ${renderGeneralDocs(sheet)}
 ${renderDataHealthReport()}</section>`;
}
...
function showSchool(viewId) {
 ...
 if (topbarTitle) {
 topbarTitle.textContent = currentVisaType === 'D4-1'? 'Thông tin trường Visa D4-1': 'Thông tin trường Visa D2-6';
 }
